...
import os
import sys
os.add_dll_directory("D:/Shengruyu/openslide-win64-20171122/bin")
import json
import openslide
...
class DeepZoomImageTiler(object):
    ...
    def __init__(self, dz, basename, format, associated, queue, slide, basenameJPG, xmlfile, mask_type, xmlLabel, ROIpc,
                 ImgExtension, SaveMasks, Mag, normalize, Fieldxml):
        self._dz = dz
            ...
            for level in range(self._dz.level_count - 1, -1, -1):
                ...
                if self._Mag > 0:
                    if ThisMag != self._Mag:
                        continue
                ########################################
                tiledir = os.path.join("%s_%sxTiles" % (self._basename, int(ThisMag)))
                ...
                for row in range(rows):
                    for col in range(cols):
                        ...
                        if InsertBaseName:
                            tilename = os.path.join(tiledir, '%s-%s_%d_%d.%s' % (ImgID, self._basenameJPG, col, row, self._format))
                            tilename_bw = os.path.join(tiledir, '%s-%s_%d_%d_mask.%s' % (ImgID, self._basenameJPG, col, row, self._format))
                        else:
                            tilename = os.path.join(tiledir, '%s-%d_%d.%s' % (ImgID, col, row, self._format))
                            tilename_bw = os.path.join(tiledir, '%s-%d_%d_mask.%s' % (ImgID,col, row, self._format))
                        if xml_valid:
    ...
    def xml_read(self, xmldir, Attribute_Name, Fieldxml):
        ...
        if Attribute_Name == "non_selected_regions":
            ...
            Image.fromarray(255 - mask).save(os.path.join(os.path.split(self._basename[:-1])[0],
            ...
            if self._mask_type == 0:
                ...
                Image.fromarray(mask).save(os.path.join(os.path.split(self._basename[:-1])[0],
                                                        "mask_" + os.path.basename(
                                                            self._basename) + "_" + Attribute_Name + ".jpeg"))
                # print(mask)
...
class DeepZoomStaticTiler(object):
    ...
    def _run_image(self, associated=None):
        ...
        tiler = DeepZoomImageTiler(dz, basename, self._format, associated, self._queue, self._slide, self._basenameJPG,
                                   ...
                                   self._SaveMasks, self._Mag, self._normalize, self._Fieldxml)
        tiler.run()
...
if __name__ == '__main__':
    ...
    parser.add_option('-o', '--output', metavar='NAME', dest='basename', default=r"results/patches",
        ...
        if ("dcm" in ImgExtension):
            ...
            try:
                DeepZoomStaticTiler(filename, output, opts.format, opts.tile_size, opts.overlap, opts.limit_bounds,
                                    ...
                                    opts.Mag, opts.normalize, opts.Fieldxml).run()
            except Exception as e:
                ...
                if (opts.mask_type == 1) or (opts.oLabelref != ''):
                    ...
                    try:
                        DeepZoomStaticTiler(filename, output, opts.format, opts.tile_size, opts.overlap,
                                            ...
                                            ImgExtension, opts.SaveMasks, opts.Mag, opts.normalize, opts.Fieldxml).run()
                    except Exception as e:
                        print("Failed to process file %s, error: %s" % (filename, sys.exc_info()[0]))
                        print(e)
